...
namespace centauri.generator
{
    public class NoiseLib
    {
        ...
        public static void FillPermutations(ref int[] p)
        {
            var permutation = new int[] { 151, 160, 137, 91, 90, 15, 131, 13, 201,
        ...
        static float PerlinFade(float val)
        {
            return val * val * val * (val * (val * 6 - 15) + 10);
        }

        public static float GetPerlinTwo(float x, float y, int seed, int[] p)
        {
            x += seed;
            y += seed;
            ...
            float fracX = x - wholeX;
            float fracY = y - wholeY;
            wholeX = wholeX & 255;
            wholeY = wholeY & 255;

            float fadeX = PerlinFade(fracX);
            float fadeY = PerlinFade(fracY);

            int aa = p[p[wholeX] + wholeY];
            int ab = p[p[wholeX] + wholeY + 1];
            int ba = p[p[wholeX + 1] + wholeY];
            int bb = p[p[wholeX + 1] + wholeY + 1];


            float end1 = PerlinLerp(PerlinGradTwo(aa, fracX, fracY), PerlinGradTwo(ba, fracX - 1, fracY), fadeX);
            float end2 = PerlinLerp(PerlinGradTwo(ab, fracX, fracY - 1), PerlinGradTwo(bb, fracX - 1, fracY - 1), fadeX);
            return 0.5f + PerlinLerp(end1, end2, fadeY);


        }

        public static float GetPerlinOne(float x, int seed, int[] p)
        {
            x += seed;
            ...
            float end1 = PerlinLerp(PerlinGradTwo(aa, fracX), PerlinGradTwo(ba, fracX - 1), fadeX);
            return 0.5f + end1;
        }

        static float PerlinLerp(float a, float b, float x)
        {
            return a + x * (b - a);
        }


        static float PerlinGradTwo(int hash, float x, float y = 0)
        {
            switch (hash & 0x7)
            {
        ...
        static float PerlinGradOne(int hash, float val)
        {
            switch (hash & 0x7)
            {
                case 0x0: return val;                       // Dot Prod with (1, 0)
                case 0x1: return val;
                case 0x2: return 0;                     // Dot Prod with (0, 1)
                case 0x3: return -val;
                case 0x4: return -val;                      // Dot Prod with (-1, 0)
                case 0x5: return -val;
                case 0x6: return 0;                     // Dot Prod with (0, -1)
                case 0x7: return val;
